...
class Game:
    def __init__(self):
        ...
        self.bx = W // 2
        self.lx, self.ly = W // 2, 420
        self.lvx, self.lvy = 0.0, 5.4
        self.reeling = False
        self.fish, self.jellies, self.sparks, self.bubbles, self.plankton = [], [], [], [], []
        self.pop = ""
        self.pop_t = 0
        for _ in range(70):
            self.plankton.append([
                random.randint(0, W), random.randint(200, H),
                random.uniform(0.2, 1.1), random.choice([TEAL, VIOLET, CYAN, MAG]),
            ])
        for _ in range(28):
            self.bubbles.append([
                random.randint(80, W - 80), random.randint(300, H),
                random.uniform(1.2, 3.4), random.randint(3, 8),
            ])
        self.spawn_timer = 0
    ...
    def burst(self, x, y, col, n=14):
        for _ in range(n):
            ...
            sp = random.uniform(1.6, 10)
            self.sparks.append([x, y, math.cos(a) * sp, math.sin(a) * sp, 20, col])

    def spawn_fish(self):
        side = random.choice((-1, 1))
        y = random.randint(520, 1680)
        x = -40 if side > 0 else W + 40
        spd = random.uniform(3.2, 6.4) * side
        kind = random.choice(["gold", "cyan", "mag"])
        self.fish.append([x, y, spd, kind, random.uniform(0, 6.28)])

    def spawn_jelly(self):
        self.jellies.append([
            random.randint(120, W - 120), H + 40,
            random.uniform(-0.6, 0.6), random.uniform(-2.4, -1.4),
        ])
    ...
    def autoplay(self):
        ...
        for f in self.fish:
            ...
            if d < best:
                best, target = d, f
        if self.ly > 1680 or (not self.fish and self.ly > 900):
            self.reeling = True
        elif target and self.ly < target[1] - 40:
            self.reeling = False
        if target:
    ...
    def tick(self):
        ...
        if self.spawn_timer % 22 == 0:
            self.spawn_fish()
        if self.spawn_timer % 70 == 0:
            self.spawn_jelly()
        self.bx += (self.lx - self.bx) * 0.08
        if self.reeling:
            self.lvy += ( -9.2 - self.lvy) * 0.18
        else:
            self.lvy += (6.1 - self.lvy) * 0.12
        self.lx += self.lvx
        self.ly += self.lvy
        self.lx = max(70, min(W - 70, self.lx))
        if self.ly < 340:
            self.ly = 340
            self.lvy = abs(self.lvy) * 0.2
            self.reeling = False
        if self.ly > 1760:
            self.ly = 1760
            self.reeling = True
        for f in self.fish:
            f[0] += f[2]
            f[1] += math.sin(self.t * 0.09 + f[4]) * 1.6
            f[4] += 0.05
        self.fish = [f for f in self.fish if -80 < f[0] < W + 80]
        for j in self.jellies:
            j[0] += j[2] + math.sin(self.t * 0.07 + j[1] * 0.01) * 0.8
            j[1] += j[3]
        ...
        for i, f in enumerate(self.fish):
            if math.hypot(f[0] - self.lx, f[1] - self.ly) < 38:
                caught.append(i)
                pts = 55 if f[3] == "mag" else (40 if f[3] == "gold" else 25)
                self.combo += 1
                self.score += pts + self.combo * 6
                self.catches += 1
                self.flash = 7
                self.pop = "HOOKED"
                self.pop_t = 16
        ...
        for j in self.jellies:
            if math.hypot(j[0] - self.lx, j[1] - self.ly) < 46:
                ...
                self.pop = "STUNG"
                self.pop_t = 14
                self.burst(self.lx, self.ly, ROSE, 16)
                self.reeling = True
                j[1] = -200
        ...
        for b in self.bubbles:
            ...
            if b[1] < 220:
                b[1] = H + 10
                b[0] = random.randint(80, W - 80)
        for e in self.plankton:
            e[1] -= e[2]
            e[0] += math.sin(self.t * 0.02 + e[1] * 0.01) * 0.4
            if e[1] < 180:
                e[1] = H + 8
    ...
    def draw(self, surf):
        ...
        for i in range(18):
            y0 = 180 + i * 100
            shade = (6 + i, 12 + i * 2, 28 + i * 3)
            pygame.draw.rect(surf, shade, (0, y0, W, 104))
        for e in self.plankton:
            pygame.draw.circle(surf, e[3], (int(e[0]), int(e[1])), 2)
        for b in self.bubbles:
            pygame.draw.circle(surf, (40, 80, 120), (int(b[0]), int(b[1])), b[3], 1)
        pygame.draw.rect(surf, INK, (0, 0, W, 230))
        pygame.draw.rect(surf, TEAL, (0, 226, W, 6))
        pygame.draw.polygon(surf, NAVY, [
            (self.bx - 90, 210), (self.bx + 90, 210),
            (self.bx + 70, 168), (self.bx - 70, 168),
        ])
        pygame.draw.polygon(surf, GOLD, [
            (self.bx - 22, 168), (self.bx + 22, 168), (self.bx, 128),
        ])
        ...
        pygame.draw.polygon(surf, LIME, [
            (int(self.lx) - 8, int(self.ly) + 14),
            (int(self.lx) + 8, int(self.ly) + 14),
            (int(self.lx), int(self.ly) + 30),
        ])
        for f in self.fish:
            col = GOLD if f[3] == "gold" else (CYAN if f[3] == "cyan" else MAG)
            fx, fy = int(f[0]), int(f[1])
            face = 1 if f[2] > 0 else -1
            pygame.draw.ellipse(surf, col, (fx - 22, fy - 12, 44, 24))
            pygame.draw.polygon(surf, col, [
                (fx - 22 * face, fy), (fx - 38 * face, fy - 12), (fx - 38 * face, fy + 12),
            ])
            pygame.draw.circle(surf, WHITE, (fx + 10 * face, fy - 2), 4)
            pygame.draw.circle(surf, VOID, (fx + 12 * face, fy - 2), 2)
        for j in self.jellies:
            jx, jy = int(j[0]), int(j[1])
            pygame.draw.ellipse(surf, VIOLET, (jx - 28, jy - 18, 56, 36), 3)
            pygame.draw.ellipse(surf, MAG, (jx - 16, jy - 10, 32, 22))
            for k in range(5):
                tx = jx - 20 + k * 10
        ...
        if self.flash:
            ...
            ov.fill((255, 196, 64, 28) if self.pop != "STUNG" else (255, 64, 120, 36))
            surf.blit(ov, (0, 0))
        if self.pop_t:
            col = ROSE if self.pop == "STUNG" else GOLD
            lab = self.font_lg.render(self.pop, True, col)
